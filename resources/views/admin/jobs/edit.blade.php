        ...
        <div class="container py-5">
            ...
            <div class="row">
                ...
                <div class="col-lg-9">
                    ...
                    <form action="{{ route('admin.jobs.update', $job->id) }}" method="post" name="editJobForm" id="editJobForm">
                        @csrf
                       @method('PUT')
                        <div class="card border-0 shadow mb-4 ">
                            <div class="card-body card-form p-4">
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea class="textarea" name="description" id="description" cols="5" rows="5" placeholder="Description">
                                        {{ $job->description }}
                                    </textarea>
                                    <p></p>
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea class="textarea" name="benefits" id="benefits" cols="5" rows="5" placeholder="Benefits">
                                        {{ $job->benefits }}
                                    </textarea>
                                    <p></p>
                                ...
                                <div class="mb-4">
                                    ...
                                    <select name="experience" id="experience" class="form-control">
                                        ...
                                        <option value="1" {{ $job->experience == 1 ? 'selected' : '' }}>1yr</option>
                                        <option value="2" {{ $job->experience == 2 ? 'selected' : '' }}>2yr</option>
                                        <option value="3" {{ $job->experience == 3 ? 'selected' : '' }}>3yr</option>
                                        <option value="4"{{ $job->experience == 4 ? 'selected' : '' }}>4yr</option>
                                        <option value="5"{{ $job->experience == 5 ? 'selected' : '' }}>5yr</option>
                                        <option value="6"{{ $job->experience == 6 ? 'selected' : '' }}>6yr</option>
                                        <option value="7"{{ $job->experience == 7 ? 'selected' : '' }}>7yr</option>
...
<script>
    $('#editJobForm').submit(function(e) {
        ...
        $.ajax({
            url: '{{ route('admin.jobs.update', $job->id) }}', // Use correct route for update
            type: 'PUT', // Correct method for update
            dataType: 'json',
            data: $('#editJobForm').serializeArray(), // Correct form ID and serialize data
            success: function(response) {
                $("button[type='submit']").prop('disabled', false);
                if (response.status === true) {
                    window.redirect = "{{ route('admin.jobs') }}";
                } else {
